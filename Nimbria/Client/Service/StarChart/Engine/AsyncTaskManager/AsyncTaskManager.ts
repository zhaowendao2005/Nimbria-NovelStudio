...
export class AsyncTaskManager {
    ...
    try {
      for (let i = 0; i < this.workerCount; i++) {
        const worker = new Worker(
          new URL('../../Workers/StarChartWorker/starChart.worker.ts', import.meta.url),
          { type: 'module' }
        )
  ...
  private executeTask(task: AsyncTask, worker: Worker): void {
    ...
    const message = {
      taskId: task.id,
      type: task.type,  // 任务类型（layout, spatial-index 等）
  ...
  private handleWorkerMessage(message: any): void {
    const { taskId, type: msgType, data, progress, error } = message

    const task = this.runningTasks.get(taskId)
    if (!task) {
      console.warn('[AsyncTaskManager] 收到未知任务的消息:', taskId)
      return
    }

    switch (msgType) {
      case 'progress':
        task.progress = progress as number
        this.notifyListeners(taskId, 'progress', { progress: progress as number })
        break
      ...
      case 'result':
        ...
        // 释放 Worker
        const resultWorker = this.findWorkerByTask(taskId)
        if (resultWorker) {
          this.workerAvailable.add(resultWorker)
        }
        break
      ...
      case 'error':
        ...
        this.runningTasks.delete(taskId)
        this.notifyListeners(taskId, 'error', { error: error as Error })
        
        // 释放 Worker
        const errorWorker = this.findWorkerByTask(taskId)
        if (errorWorker) {
          this.workerAvailable.add(errorWorker)
        }
        break

      default:
        console.warn('[AsyncTaskManager] 未知消息类型:', msgType)
    }
