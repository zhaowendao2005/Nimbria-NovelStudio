 ...
 * 管理应用程序缓存相关的状态
 * 从原 settings.store.ts 中拆分出来
 */
export const useSettingsCacheStore = defineStore('settings-cache', () => {
  /**
  ...
  function clearAllCache() {
    console.log('💾 [Settings Cache Store] 执行 localStorage.clear()...')
    const beforeCount = localStorage.length
    
    localStorage.clear()
    
    const afterCount = localStorage.length
    console.log(`💾 [Settings Cache Store] localStorage 已清空`)
    console.log(`   清理前: ${beforeCount} 项`)
    console.log(`   清理后: ${afterCount} 项`)
    
    if (afterCount === 0) {
      console.log(`✅ [Settings Cache Store] localStorage 清理验证通过`)
    } else {
      console.warn(`⚠️  [Settings Cache Store] localStorage 仍有 ${afterCount} 项残留`)
    }
  }
