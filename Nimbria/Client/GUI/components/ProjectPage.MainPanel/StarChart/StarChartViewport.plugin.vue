...
const initGraph = async () => {
  ...
  // ===== 2. 执行布局计算 =====
  const layoutResult = await plugin.executeWithHooks(adaptedData, {
  ...
  // ===== 4. 注册自定义布局到G6（如果需要） =====
  // 对于 multi-root-radial，我们需要将它注册为G6布局
  if (!isG6NativeLayout(layout.name)) {
    registerCustomLayout(plugin, layoutResult)
  }
  ...
  const graphConfig: any = {
    ...
    edge: {
      type: (edge: any) => {
        const edgeStyle = finalStyles.edge(edge)
        return edgeStyle.type || 'line'
      },
 ...
 * 检查是否是G6原生布局
 */
function isG6NativeLayout(layoutName: string): boolean {
  const nativeLayouts = ['force', 'circular', 'dagre', 'grid', 'concentric']
  return nativeLayouts.includes(layoutName)
}

/**
 * 注册自定义布局到G6
 */
function registerCustomLayout(plugin: any, layoutResult: any): void {
  // 这里我们已经计算好了位置，所以使用preset布局即可
  // 无需额外注册
}
